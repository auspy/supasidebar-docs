...
| **`Cmd Opt` click** | Replaces the active browser tab with this link instead of opening a new tab (Beta). Falls back to a normal open if the browser can't replace the active tab (Firefox family, Orion, Zen) |
| **`Opt` click** | Shows a browser and profile picker popover so you can choose which browser or profile to open in |
...
- **Press `Opt Return` on a result** to switch to browser-selection mode and pick a specific browser or profile
...
| **Open With** | Shows a submenu with all installed browsers and their profiles to choose from |
