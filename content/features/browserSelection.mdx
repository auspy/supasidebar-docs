...
## Goals
- Make it easy for non‑technical users to pick a browser for each link.
...
1. **Right-click → "Open with" (Set / Change assignment)**
...
2. **Modifier key + Click / Enter — One-time choice**
   - One-time (temporary): Does not change the saved assignment.
   - Mouse: Hold **Option (⌥)** and click the link → Browser picker appears.
   - Keyboard: With a link selected, press **Option (⌥) + Enter** → Browser picker appears.

3. **Command panel / Quick Open → Option + Enter**
   - Quick workflow for power users: open the command panel (⌘⌃K), search for a link, press **⌥Enter** to show the picker, then select browser with ↑/↓ + Return.
...
- **Help text**: first-time tooltip explaining `Option + click` and `Right‑click → Open with`.
