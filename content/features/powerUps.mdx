...
Once a link has Auto-update enabled in template mode (with a tracked number), the link's right-click menu adds a third action after **Open** and **Open With**: **Open Next episode**. It opens the next-numbered URL (current episode + 1) in the frontmost browser, without changing what the saved link points at - the regular 30-second dwell rule still decides when the link itself advances.
