...
## Ask AI about a link or folder

Send one specific link or folder straight into a chat instead of typing a question from scratch:

1. Open the Command Panel with `Cmd Ctrl K` and select a link, folder, or search result
2. Press `Opt Return` to open the actions panel
3. Choose **Ask AI**

SupaSidebar activates AI mode with a starter question already filled in - "Tell me about [name]" for a folder or app, or "Tell me about [name] ([URL])" for a web link - so the conversation opens already anchored to that item instead of a blank prompt.
